...
def calculate_distance_m(pos_a_km: np.ndarray, pos_b_km: np.ndarray) -> float:
    ...
    return math.sqrt(distance_km_sq) * 1000.0 # Convert km to m

def calculate_delay_ms(pos_a_km: np.ndarray, pos_b_km: np.ndarray,
                       proc_delay_ms: float, prop_coef: float, prop_speed_km_s: float) -> float:
    """
    ...
    prop_delay_ms = prop_coef * dist_m / prop_speed_km_s
