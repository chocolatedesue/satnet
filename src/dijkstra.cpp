...
// Note: Assumes N, GlobalConfig::sat_pos, move, cur_banned, futr_banned, route_table
...
double DijkstraNode::getDist(int id_a, int id_b) {
    ...
    for (int i = 0; i < 3; ++i) { // Assuming 3D coordinates
        double d = GlobalConfig::sat_pos.at(id_a)[i] - GlobalConfig::sat_pos.at(id_b)[i];
        res_sq += d * d;
    }
...
double DijkstraNode::calcuDelay(int a, int b) {
    ...
    // proc + prop delay (ms)
    return GlobalConfig::proc_delay + GlobalConfig::prop_delay_coef * distance / GlobalConfig::prop_speed * 1000.0;
}

std::string DijkstraNode::getName() {
    return "DijkstraBase";
}
...
void DijkstraNode::compute() {
    ...
    while (!pq.empty()) {
        ...
        if (d_u > dist[u]) continue; // Skip if found shorter path already
        vis[u] = 1; 

        // Explore neighbors (ports 1-4)
        for (int port = 1; port <= 4; ++port) {
            int v = move(u, port); 
            if (v < 0 || v >= GlobalConfig::N) continue; 

            double w = calcuDelay(u, v); 
            if (w == std::numeric_limits<double>::max()) continue; 
...
DijkstraProbeNode::DijkstraProbeNode(int id)
    : DijkstraNode(id) {} 

std::string DijkstraProbeNode::getName() {
...
void DijkstraProbeNode::computeWithBannedPorts(const std::vector<std::array<int, 5>>* banned_ptr) {
...
DijkstraPredNode::DijkstraPredNode(int id)
    : DijkstraProbeNode(id) {} 

std::string DijkstraPredNode::getName() {
    return "DijkstraPred";
}
