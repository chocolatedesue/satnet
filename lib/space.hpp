...
public:
  SpaceSimulation(std::string config_file_name) {
    ...
    // 如果 report_dir 不存在，则创建
    if (opendir(report_dir.c_str()) == NULL) {
      mkdir(report_dir.c_str(), S_IRWXU | S_IRWXG | S_IRWXO);
      printf("report_dir: %s created\n", report_dir.c_str());
    } else
      printf("report_dir: %s\n", report_dir.c_str());
    ...
    sprintf(report_filename, "report [%s] %s.txt", name.c_str(),
            algorithm.c_str());
    /*
    sprintf(report_filename, "report [%s] %s %d-%02d-%02d %02d:%02d:%02d.txt",
        name.c_str(), algorithm.c_str(), my_tm->tm_year + 1900, my_tm->tm_mon +
    1, my_tm->tm_mday, my_tm->tm_hour, my_tm->tm_min, my_tm->tm_sec);
    */
  }
...
private:
  ...
  void dump_nodes() {
    ...
    for (int i = 0; i < N; i++) {
      ...
      frame_nodes_3d.push_back(
          std::vector<double>({sat_pos[i][0], sat_pos[i][1], sat_pos[i][2]}));
    }
  }
  ...
  void dump_dawn_dust_line() {
    ...
    std::ifstream icrs_ifs(dawn_dusk_icrs_dir + "/" + std::to_string(cur_time) +
                           ".txt");
    ...
    double x, y, z;
    while (icrs_ifs >> x >> y >> z &&
           frame_nodes_3d.size() < frame_nodes.size()) {
  ...
  void save_frame() {
    ...
    for (auto node : frame_nodes) {
      ...
      fprintf(fout, "%f %f %d", node.first.first, node.first.second,
              node.second);
    }
    fprintf(fout, "\n");
    first = true;
    for (auto edge : frame_edges) {
      ...
      fprintf(fout, "%d %d %d", edge.first.first, edge.first.second,
              edge.second);
    }
    fprintf(fout, "\n");
    first = true;
  ...
  void load_sat_lla() {
    using namespace GlobalConfig;
    std::string sat_lla_filename =
        sat_lla_dir + "/" + std::to_string(cur_time) + ".csv";
  ...
  void load_sat_pos() {
    using namespace GlobalConfig;
    auto ifs =
        std::ifstream(sat_pos_dir + "/" + std::to_string(cur_time) + ".csv");
    for (int i = 0; i < N; i++) {
  ...
  void load_sat_vel() {
    auto ifs =
        std::ifstream(sat_vel_dir + "/" + std::to_string(cur_time) + ".csv");
    for (int i = 0; i < N; i++) {
      ifs >> sat_vel[i];
    }
  }
